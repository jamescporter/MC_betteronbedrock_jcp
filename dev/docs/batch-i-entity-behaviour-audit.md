...
| `entities/bosses/seeker/seeker.behavior.json` | Retained | Kept the `bob:seeker_active` spawn tag for both boss and end seeker script filtering. The randomised boss component groups and variant-driven beam script hooks were checked together with the launcher and dummy behaviours. |
...
| `entities/end_mobs/seeker.behavior.json` | Retained | Kept the `bob:seeker_active` spawn tag, slower attack timers, and narrowed projectile-only cleanup. The 1.2.1 broad `event entity @e[r=5] despawn` cleanup was not ported because it can affect unrelated nearby helpers/entities, whereas current cleanup targets the projectile helper explicitly. |
...
2. End seeker: passive/random attack cycle timing, beam/charge behaviour, explicit projectile cleanup, and script-applied damage/debuffs.
