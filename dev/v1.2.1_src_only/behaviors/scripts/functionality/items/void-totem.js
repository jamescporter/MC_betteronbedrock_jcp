...
export async function voidTotem(player) {
    if (!player?.isValid)
        return;
    ...
    await player.applyKnockback({ x: 0, z: 0 }, 7.3);
    ...
    equipment.setEquipment(mainhand?.typeId === "better_on_bedrock:void_totem" ? EquipmentSlot.Mainhand : EquipmentSlot.Offhand, undefined);
};