...
world.afterEvents.itemReleaseUse.subscribe((data) => {
    ...
    if (tridentData.riptide && enchComp?.hasEnchantment("riptide")) {
        ...
        if (TridentManager.isInEnvironment(riptide.environment, source)) {
            ...
            const baseForce = riptide.velocity + ((riptide.velocity / 6) * level);

            const horizontalStrength =
                (Math.abs(viewDir.x) + Math.abs(viewDir.z)) * 1.5 * baseForce;
            ...
            const verticalStrength = viewDir.y * baseForce;
            source.applyKnockback(horizontal, verticalStrength);
            ...
            switch (enchant.id) {
                ...
                case "knockback":
                    if (entity?.isValid && entityLoc)
                        try {
                            const dx = entityLoc.x - projLoc.x;
                            const dz = entityLoc.z - projLoc.z;

                            const force = 1.25 * enchant.lvl;
                            ...
                            const vertical = 0.2 * enchant.lvl;
